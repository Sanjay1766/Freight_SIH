...
export default function RiskConeChart({ historySeries, forecastData, volatilityStats, selectedHorizon, onSelectHorizon }) {
  ...
  return (
    <div className="card-clean p-6 relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        ...
        <div className="flex items-center gap-3 bg-slate-50 px-3.5 py-2 rounded-xl border border-slate-200">
          <div className="text-right">
            <div className="text-[10px] uppercase font-bold text-slate-400">Daily Vol (σ)</div>
            <div className="text-xs font-mono font-bold text-[#FF3B00]">{volatilityStats.dailyVol}%</div>
          </div>
          <div className="h-6 w-px bg-slate-200" />
          <div className="text-right">
            <div className="text-[10px] uppercase font-bold text-slate-400">Annual Vol</div>
      ...
      {/* Horizon Summary Bar */}
      <div className="mb-4 p-3.5 rounded-xl bg-orange-500/5 border border-orange-500/20 flex flex-wrap items-center justify-between gap-2 text-xs">
        ...
        <div className="flex items-center gap-4 font-mono">
          <div>Point Rate: <span className="text-[#FF3B00] font-bold">${currentForecastAtHorizon.pointForecast.toLocaleString()}/day</span></div>
          <div>95% Bound: <span className="text-slate-600">${currentForecastAtHorizon.lower95.toLocaleString()} - ${currentForecastAtHorizon.upper95.toLocaleString()}</span></div>
        </div>
      </div>
      ...
      <div className="mt-4 flex items-center justify-between gap-2 border-t border-slate-100 pt-3 text-xs">
        <span className="text-slate-500 font-semibold">Forecast Horizon:</span>
