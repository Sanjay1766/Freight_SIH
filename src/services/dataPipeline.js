 ...
 *
 * Implements:
 * 1. Multi-source proxy series generation & time-alignment
...
function cubicSplineInterpolate(monthlyPoints, totalDays) {
  ...
  // Simple smooth monotonic cubic spline approximation for daily values
  const daily = [];
  for (let d = 0; d < totalDays; d++) {
    ...
    // Divide monthly total by 30 days to get daily rate in metric tons
...
export function generateHistoricalData(scenarioModifiers = {}) {
  ...
  for (let i = 0; i < totalDays; i++) {
    ...
    currentBDI = Math.max(1100, Math.min(3800, currentBDI + rand1));
    ...
    if (regime === 'monsoon' && i > 60) {
      regimeMultiplier = 1.25; // Port delays spike effective tightness
    ...
    // Normalized to scale ~ 1.0 - 2.5 for easy interpretation
